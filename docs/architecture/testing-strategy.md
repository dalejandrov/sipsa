...
| Integration tests (handler-level, real SOAP transport + real DB) | **2 of 5.** `CiudadIngestionHandlerIT` ([TECH-151](../backlog/technical-backlog.md#tech-151)) and `SemanaIngestionHandlerIT` ([TECH-152](../backlog/technical-backlog.md#tech-152), which additionally exercises both of Semana's real upsert paths — `upsertTmpBatch` and TECH-060's `upsertFallbackBatch`), both 2026-08-03 — golden path, idempotency, and SOAP-fault cases, through the real `SoapGatewayImpl`/`SoapStreamingClient` and a real Testcontainers PostgreSQL. The other 3 handlers (`Mes`, `Abas`, `Parcial`) are tracked as [TECH-153..155](../backlog/technical-backlog.md#tech-153); `ParcialIngestionHandlerTest` still only covers the mocked-repo/no-real-transport path (kept as-is, see ADR-011) |
...
- `CiudadIngestionHandlerIT` (TECH-151) and `SemanaIngestionHandlerIT` (TECH-152) —
  **done**, both 2026-08-03. `MesIngestionHandlerIT`, `AbasIngestionHandlerIT`,
  `ParcialIngestionHandlerIT` — pending (TECH-153..155), same pattern.
