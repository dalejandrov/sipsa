 ...
 * {@link SipsaMayoristasSemanalBatchInsertRepositoryImpl}'s TECH-060 technique (business-key
 * sibling of the same table).
 * <p>
 ...
 * whole batch — and costs one round trip per row. {@code ON CONFLICT (tmp_mayo_sem_id)
 * DO NOTHING} (backed by the existing {@code ux_semana_tmp} constraint) resolves both
...
public class SipsaMayoristasSemanalTmpBatchInsertRepositoryImpl implements SipsaMayoristasSemanalTmpBatchInsertRepository {
    ...
    private static final String INSERT_SQL = """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT (tmp_mayo_sem_id) DO NOTHING""";

    private final JdbcTemplate jdbcTemplate;

    @Override
    public int[] insertIgnoringTmpConflicts(List<SipsaMayoristasSemanal> rows) {
