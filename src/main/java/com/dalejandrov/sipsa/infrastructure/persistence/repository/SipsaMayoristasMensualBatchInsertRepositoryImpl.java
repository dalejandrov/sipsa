...
/**
 * JDBC implementation of the concurrency-safe fallback batch insert, mirroring
 * {@link SipsaMayoristasSemanalBatchInsertRepositoryImpl}'s TECH-060 technique.
 * <p>
 ...
 * fecha_mes_ini) DO NOTHING} (backed by the existing {@code ux_mes_fallback} constraint)
...
public class SipsaMayoristasMensualBatchInsertRepositoryImpl implements SipsaMayoristasMensualBatchInsertRepository {
    ...
    private static final String INSERT_SQL = """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT (arti_id, fuen_id, fecha_mes_ini) DO NOTHING""";

    private final JdbcTemplate jdbcTemplate;

    @Override
    public int[] insertIgnoringConflicts(List<SipsaMayoristasMensual> rows) {
    ...
    private static void bindRow(PreparedStatement ps, SipsaMayoristasMensual row) throws SQLException {
        setNullableLong(ps, 1, row.getTmpMayoMesId());
