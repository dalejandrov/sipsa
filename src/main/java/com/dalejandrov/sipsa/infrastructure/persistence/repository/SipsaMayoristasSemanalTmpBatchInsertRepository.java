 ...
 * {@link SipsaMayoristasSemanalBatchInsertRepository} (its business-key sibling, TECH-060)
 * and {@link SipsaParcialBatchInsertRepository} (TECH-117).
 * <p>
 * The insert is atomic per row inside PostgreSQL — {@code ON CONFLICT (tmp_mayo_sem_id)
 * DO NOTHING}, backed by the {@code ux_semana_tmp} unique constraint — so a concurrent
 * writer racing between the batch's in-memory dedup and the insert no longer produces a
 ...
 *
 * @see SipsaMayoristasSemanalRepository#upsertTmpBatch(List)
 */
public interface SipsaMayoristasSemanalTmpBatchInsertRepository {

    /**
     * Inserts the given rows with {@code INSERT … ON CONFLICT (tmp_mayo_sem_id)
     ...
     * <p>
     * A row with a {@code null} {@code tmpMayoSemId} can never match the conflict target
     ...
     *         inserted, {@code 0} if it was dropped by a {@code tmp_mayo_sem_id} conflict
     */
    int[] insertIgnoringTmpConflicts(List<SipsaMayoristasSemanal> rows);
}
