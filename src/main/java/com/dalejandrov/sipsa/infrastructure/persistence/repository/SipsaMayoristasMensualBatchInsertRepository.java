 ...
 * {@link SipsaMayoristasSemanalBatchInsertRepository} (TECH-060).
 * <p>
 * The insert is atomic per row inside PostgreSQL — {@code ON CONFLICT (arti_id, fuen_id,
 * fecha_mes_ini) DO NOTHING}, backed by the {@code ux_mes_fallback} unique constraint — so
 ...
 * usable.
 *
 * @see SipsaMayoristasMensualRepository#upsertFallbackBatch(List)
 */
public interface SipsaMayoristasMensualBatchInsertRepository {
     ...
     */
    int[] insertIgnoringConflicts(List<SipsaMayoristasMensual> rows);
}
