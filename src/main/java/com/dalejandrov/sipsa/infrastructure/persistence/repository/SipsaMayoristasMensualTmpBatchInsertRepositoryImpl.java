 ...
 * {@link SipsaMayoristasSemanalTmpBatchInsertRepositoryImpl}'s technique.
 * <p>
 ...
 * whole batch — and costs one round trip per row. {@code ON CONFLICT (tmp_mayo_mes_id)
 * DO NOTHING} (backed by the existing {@code ux_mes_tmp} constraint) resolves both
...
public class SipsaMayoristasMensualTmpBatchInsertRepositoryImpl implements SipsaMayoristasMensualTmpBatchInsertRepository {
    ...
    private static final String INSERT_SQL = """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT (tmp_mayo_mes_id) DO NOTHING""";

    private final JdbcTemplate jdbcTemplate;

    @Override
    public int[] insertIgnoringTmpConflicts(List<SipsaMayoristasMensual> rows) {
