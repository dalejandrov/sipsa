 ...
 * {@link SipsaMayoristasSemanalTmpBatchInsertRepository} (TECH-060/TECH-117 lineage).
 * <p>
 * The insert is atomic per row inside PostgreSQL — {@code ON CONFLICT (tmp_mayo_mes_id)
 * DO NOTHING}, backed by the {@code ux_mes_tmp} unique constraint — so a concurrent writer
 ...
 *
 * @see SipsaMayoristasMensualRepository#upsertTmpBatch(List)
 */
public interface SipsaMayoristasMensualTmpBatchInsertRepository {

    /**
     * Inserts the given rows with {@code INSERT … ON CONFLICT (tmp_mayo_mes_id)
     ...
     * <p>
     * A row with a {@code null} {@code tmpMayoMesId} can never match the conflict target
     ...
     *         inserted, {@code 0} if it was dropped by a {@code tmp_mayo_mes_id} conflict
     */
    int[] insertIgnoringTmpConflicts(List<SipsaMayoristasMensual> rows);
}
